...
const NAV_ITEMS = [
  {
    ...
    href: '/appointments',
    icon: (active: boolean) => (
      <svg viewBox="0 0 24 24" fill="currentColor" className={`w-5 h-5 shrink-0 ${active ? 'text-[#6BC9E4]' : ''}`}>
          ...
          return (
            <Link
              ...
              className={`flex items-center gap-3 px-3 py-3 rounded-xl text-sm font-medium transition-colors ${
                active
                  ? 'bg-[#EDF8FC] dark:bg-slate-700 text-[#6BC9E4]'
            ...
            <li key={key}>
              <Link
                ...
                className={`flex flex-col items-center gap-0.5 px-2 py-2 rounded-lg text-xs font-medium transition-colors ${
                  active ? 'text-[#6BC9E4]' : 'text-gray-800 dark:text-gray-200'
                }`}
              >
                {icon(active)}
